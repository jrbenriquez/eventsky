...
- opens n concurrent sse connections to /events/{code}/stream
...
usage:
  python sse_load_test.py --base-url https://your.app --code abc123 \
      ...
      --post-path /events/{code}/send  --concurrency 5
    ...
    --stream-path /events/{code}/stream \
    --post-path /events/{code}/message \
    --timeout 40
...
def default_post_json(run_id: str, msg_id: int):
    ...
    return {"text": f"[lt-{run_id}] message #{msg_id}", "sender_name": "loadtester"}
...
def parse_args():
    ...
    p.add_argument("--stream-path", default="/events/{code}/stream", help="sse stream path")
    p.add_argument(
        "--post-path", default="/events/{code}/message", help="post path to create/send a message"
    )
...
async def open_sse(
    session: aiohttp.clientsession, url: str, client_id: int, run_marker: str, inbox: set
):
    """
...
async def main():
    ...
    async with aiohttp.clientsession(timeout=timeout, connector=conn) as session:
        ...
        while time.time() < deadline:
            all_ok = all(
                expected_strings.issubset(inbox_texts(client_inboxes[i]))
                for i in range(args.clients)
            )
            if all_ok:
                break
        ...
        # report
        received_counts = [
            len(expected_strings.intersection(inbox_texts(client_inboxes[i])))
            for i in range(args.clients)
        ]
        ...
        if full_receivers < args.clients:
            missing = [
                (i, total_msgs - received_counts[i])
                for i in range(args.clients)
                if received_counts[i] < total_msgs
            ]
            print(
                "clients with missing messages (client_id -> missing count):",
                missing[:20],
